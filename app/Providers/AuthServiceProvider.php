...
class AuthServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        // Auth gates for: Wings
        Gate::define('wing_access', function ($user) {
            return in_array($user->role_id, [1, 2, 3]);
        });
        Gate::define('wing_create', function ($user) {
            return in_array($user->role_id, [1]);
        });
        Gate::define('wing_edit', function ($user) {
            return in_array($user->role_id, [1]);
        });
        Gate::define('wing_view', function ($user) {
            return in_array($user->role_id, [1, 2, 3]);
        });
        Gate::define('wing_delete', function ($user) {
            return in_array($user->role_id, [1]);
        });

        // Auth gates for: Buildings
        Gate::define('building_access', function ($user) {
            return in_array($user->role_id, [1,2,3]);
        });
        Gate::define('building_create', function ($user) {
            return in_array($user->role_id, [1]);
        });
        Gate::define('building_edit', function ($user) {
            return in_array($user->role_id, [1]);
        });
        Gate::define('building_view', function ($user) {
            return in_array($user->role_id, [1, 2, 3]);
        });
        Gate::define('building_delete', function ($user) {
            return in_array($user->role_id, [1]);
        });
