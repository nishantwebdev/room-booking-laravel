                    ...
                    @if (count($categories) > 0)
                        @foreach ($categories as $country)
                            <tr data-entry-id="{{ $country->id }}">
                                ...
                                @endcan
                                <td field-key=''>{{ $country->id }}</td>
                                <td field-key='name'>{{ $country->name }}</td>
                                <td field-key='name'>{{ $country->label }}</td>
                                <td field-key='name'>@if($country->image)<img src="{{ asset('public/images/category/'.$country->image) }}" title="Image" style="width: 50px;" />@else N/A  @endif</td>
                                <td field-key='name'>{{ $country->name }}</td>
                                @if( request('show_deleted') == 1 )
                                <td>
                                    @can('country_delete')
